...
function guardar() {
    try { localStorage.setItem('mat_activos', JSON.stringify(state)); } catch (_) { }
    GistSync.subirAuto();
    AnioCombo.poblar();
}
function cargar() {
    try {
        const raw = parseSeguro(localStorage.getItem('mat_activos') || 'null');
        if (raw) {
    ...
    try {
        const viejas = parseSeguro(localStorage.getItem('mat_herramientas') || 'null');
        if (Array.isArray(viejas) && viejas.length) {
            ...
            localStorage.removeItem('mat_herramientas');
        }
    } catch (_) { }
}
...
document.getElementById('confirmar-ok').onclick = () => {
...
document.getElementById('confirmar-cancelar').onclick = () => _volverAlPadre();
...
function toggleDarkMode() {
    ...
    if (btn) {
        ...
        if (iconUse) {
            // Si está en dark mode, mostramos el sol para volver al claro, y viceversa
            iconUse.setAttribute('href', dark ? '#icon-sun' : '#icon-moon');
        }
    }
    try { localStorage.setItem('mat_dark', dark ? '1' : '0'); } catch (_) { }
}
...
function onImportarFileChange(e) {
    ...
    if (file.size > MAX_IMPORT_MB * 1024 * 1024) {
        ...
        label.innerHTML = `<strong class="import-fail">✗ Archivo demasiado grande</strong><br><span class="import-sub">El máximo permitido es ${MAX_IMPORT_MB} MB.</span>`;
    ...
    reader.onload = async ev => {
        try {
            ...
            const textoDesc = hayDescartados
                ? `<br><span class="text-red-sm">⚠ Descartados por formato inválido: ${partesDesc.join(' · ')}</span>`
                : '';
            ...
            if (esValida) {
                label.innerHTML = `<strong class="import-ok">✓ ${esc(file.name)}</strong><br><span class="import-sub">${parsed.materiales.length} materiales · ${parsed.movimientos.length} movimientos${herrLabel}</span>${textoDesc}`;
                zone.style.borderColor = hayDescartados ? 'var(--c-orange)' : 'var(--c-green)';
            } else {
                label.innerHTML = `<strong class="text-orange">⚠️ Archivo modificado externamente</strong><br><span class="import-sub">${parsed.materiales.length} materiales · ${parsed.movimientos.length} movimientos${herrLabel}</span>${textoDesc}`;
            ...
            _importarParsed = null;
            label.innerHTML = `<strong class="import-fail">✗ Archivo inválido</strong><br><span class="import-sub">El archivo no tiene el formato correcto o está dañado.</span>`;
            zone.style.borderColor = 'var(--c-red)';
            btn.disabled = true;
            btnCombinar.disabled = true;
        }
...
function importarDatos(modo) {
    ...
    if (modo === 'reemplazar') {
        ...
        confirmar('¿Combinar datos?', alerta + 'Se agregarán los materiales y movimientos del archivo que no existan actualmente.', () => {
            ...
            // Pre-conteo sin mutar
            let _cMats = 0, _cMovs = 0, _cCats = 0, _cHerr = 0;
            (parsed.materiales || []).forEach(m => {
                if (!matIds.has(m.id)) { _cMats++; }
                else {
                    const ex = state.materiales.find(x => x.id === m.id);
                    if (ex && ((ex.umbralBajo === null && m.umbralBajo !== null) || (ex.umbralAlto === null && m.umbralAlto !== null))) _cMats++;
                }
            });
            (parsed.movimientos || []).forEach(m => { if (!movIds.has(m.id)) _cMovs++; });
            (parsed.categorias || []).forEach(c => { if (!catSet.has(c.toLowerCase())) _cCats++; });
            (parsed.herramientas || []).forEach(h => { if (!herrIds.has(h.id)) _cHerr++; });
...
function buscarMovimientosMaterialEditar() {
    ...
    // 2. Preparamos el texto de búsqueda. Si hay filtro de año, armamos el rango (ej: 01/2020 12/2020)
    let textoBusqueda = nombre;
    if (_anioFiltro) {
        textoBusqueda = `${nombre} 01/${_anioFiltro} 12/${_anioFiltro}`;
    }
...
//  HERRAMIENTAS
// ═══════════════════════════════════════════════════════
function _cargarHerramientas() {
    return state.herramientas || [];
}

function _guardarHerramientas(lista) {
    state.herramientas = lista;
    guardar();
}

function renderHerramientas() {
    const lista = _cargarHerramientas();
...
function agregarHerramienta() {
    ...
    const nombreEl = document.getElementById('herr-nombre-input');
    const cantidadEl = document.getElementById('herr-cantidad-input');
    const fecha = fechaEl ? fechaEl.value.trim() : '';
    const nombre = nombreEl ? nombreEl.value.trim() : '';
    const cantidadRaw = cantidadEl ? Number(cantidadEl.value) : NaN;
    const cantidad = (Number.isFinite(cantidadRaw) && cantidadRaw >= 1) ? Math.floor(cantidadRaw) : null;

    let ok = true;
    if (!fecha || !/^\d{4}-\d{2}-\d{2}$/.test(fecha)) { if (fechaEl) fechaEl.classList.add('error'); ok = false; }
    else if (fechaEl) fechaEl.classList.remove('error');
    if (!nombre) { if (nombreEl) nombreEl.classList.add('error'); ok = false; }
    else if (nombreEl) nombreEl.classList.remove('error');
    if (!cantidad) { if (cantidadEl) cantidadEl.classList.add('error'); ok = false; }
    else if (cantidadEl) cantidadEl.classList.remove('error');

    if (!ok) { toast('Completá todos los campos', 'error'); return; }

    const lista = _cargarHerramientas();
    historial.empujar(`Agregar herramienta "${nombre}"`);
    lista.push({ id: uid(), fecha, nombre: nombre.slice(0, 200), cantidad });
    _guardarHerramientas(lista);
    if (nombreEl) nombreEl.value = '';
    if (cantidadEl) cantidadEl.value = '1';
    if (!isMobile()) setTimeout(() => { if (nombreEl) nombreEl.focus(); }, 50);
    renderHerramientas();
    toast('Herramienta registrada');
}

function eliminarHerramienta(id) {
    const h = _cargarHerramientas().find(x => x.id === id);
    const label = h ? `Eliminar herramienta "${h.nombre}"` : 'Eliminar herramienta';
    confirmar('¿Eliminar herramienta?', 'Se eliminará el registro permanentemente.', () => {
        historial.empujar(label);
        const lista = _cargarHerramientas().filter(h => h.id !== id);
        _guardarHerramientas(lista);
        renderHerramientas();
...
function switchTab(tab) {
    ...
    try {
        localStorage.setItem('mat_tab', tab);
        localStorage.setItem('mat_tab_time', Date.now().toString()); // <--- Guardamos la hora exacta
    } catch (_) { }
...
const AnioCombo = (() => {
    ...
    function abrir() {
        ...
        const items = [
            ...
            ...anios.map(a => `<div class="suggestion-item${_anioFiltro === a ? ' highlighted' : ''}" data-anio="${a}">${a}</div>`)
        ];
...
function renderStats() {
    ...
    const modoAnio = !!_anioFiltro;
    const estiloLabelFiltro = modoAnio ? 'class="stat-chip-label-filtro"' : '';
    ...
    let top5Html = `
                <span class="stat-chip-label" ${estiloLabelFiltro}>Más consumidos</span>
    ...
    if (top5.length > 0) {
        top5Html = `
                    <span class="stat-chip-label fade-transition" id="top5-label" ${estiloLabelFiltro}>Más consumidos #1</span>
    ...
    document.getElementById('stats-grid').innerHTML = `
                ...
                <div class="stat-chip">
                    <span class="stat-chip-label" ${estiloLabelFiltro}><svg class="svg-icon"><use href="#icon-movements"/></svg> Movimientos</span>
                    <span class="stat-chip-value">${totalMovs}</span>
                    <span class="stat-chip-sub">${labelMovs}</span>
                </div>
                <div class="stat-chip">
                    <span class="stat-chip-label" ${estiloLabelFiltro}><svg class="svg-icon"><use href="#icon-back"/></svg> Último</span>
                    ${ultimoHtml}
                </div>
    ...
    if (top5.length > 1) {
        ...
        _intervaloTop5 = setInterval(() => {
            ...
            // Si los elementos ya no están en pantalla (ej: el usuario cambió de pestaña), cortamos el bucle
            if (!lbl || !nom || !sub) {
                clearInterval(_intervaloTop5);
                return;
            }

            // 1. Bajamos la opacidad a 0 (desvanece)
            lbl.style.opacity = 0;
            nom.style.opacity = 0;
            sub.style.opacity = 0;

            // 2. Esperamos un instante, cambiamos los textos y subimos la opacidad a 1
            setTimeout(() => {
                lbl.textContent = `Más consumidos #${idx + 1}`;
                nom.textContent = top5[idx].nombre;
                nom.title = top5[idx].nombre; // Actualiza el tooltip flotante
                sub.textContent = `${top5[idx].cantidad} unidades`;
        ...
        }, 3000); // tiempo de toggle iterar chips top ranking 5 mas usados
    }
}
...
// ═══════════════════════════════════════════════════════
// Cada modal tiene su propio array de líneas y su contador
const _lineasState = {
...
function eliminarLinea(tipo, lid) {
    _lineasState[tipo].lineas = _lineasState[tipo].lineas.filter(l => l.lid !== lid);
    renderLineas(tipo);
}
    ...
    container.innerHTML = lineas.map(l => `
        <div class="linea-material" id="linea-row-${tipo}-${l.lid}">
            <div class="combobox-wrap">
                ...
                >
                <div class="suggestions-list" id="linea-sugg-${tipo}-${l.lid}"></div>
            </div>
            <input
                type="number"
...
function onLineaInput(tipo, lid, val) {
    ...
    linea._nombre = val; linea.materialId = null;
    mostrarSugerencias(tipo, lid, val);
}

function onLineaFocus(tipo, lid) {
    const linea = _getLinea(tipo, lid); if (!linea) return;
    if (linea._nombre) mostrarSugerencias(tipo, lid, linea._nombre);
}

function mostrarSugerencias(tipo, lid, val) {
    const el = document.getElementById(`linea-sugg-${tipo}-${lid}`); if (!el) return;
    ...
    // Fix para móviles: le damos 150ms al navegador para que el teclado se acomode
    setTimeout(() => {
        // Buscamos toda la fila del material en lugar de la lista flotante (es más estable en móviles)
        const filaMaterial = document.getElementById(`linea-row-${tipo}-${lid}`);

        if (filaMaterial) {
            // Centramos la fila en la pantalla, dejando lugar perfecto para la lista abajo
            filaMaterial.scrollIntoView({ behavior: 'smooth', block: 'center' });
        } else {
...
function onLineaKey(e, tipo, lid) {
    const el = document.getElementById(`linea-sugg-${tipo}-${lid}`);
    ...
    if (e.key === 'ArrowDown') {
        e.preventDefault();
        if (hi < items.length - 1) { if (hi >= 0) items[hi].classList.remove('highlighted'); items[hi + 1].classList.add('highlighted'); }
    } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        if (hi > 0) { items[hi].classList.remove('highlighted'); items[hi - 1].classList.add('highlighted'); }
    } else if (e.key === 'Enter') {
        e.preventDefault();
        e.stopPropagation();
...
function seleccionarMaterial(tipo, lid, materialId) {
    ...
    const suggEl = document.getElementById(`linea-sugg-${tipo}-${lid}`);
    if (inputEl) inputEl.value = mat.nombre;
    if (suggEl) suggEl.classList.remove('show');
...
document.addEventListener('mousedown', e => {
    document.querySelectorAll('.suggestions-list.show').forEach(el => {
        const wrap = el.closest('.combobox-wrap');
        if (wrap && !wrap.contains(e.target)) el.classList.remove('show');
    });
});
...
document.addEventListener('click', e => {
    ...
    if (ys) { ys.parentElement.classList.toggle('open'); return; }
...
function guardarMovimiento(tipo) {
    ...
    if (!ticket) { document.getElementById(ticketId).classList.add('error'); toast('Ingresá un número de ticket', 'error'); return; }
...
function guardarEdicionMov() {
    ...
    if (!nuevaFecha) { toast('La fecha es obligatoria', 'error'); return; }
...
function renderMovimientos() {
    ...
    const htmlFinal = Object.keys(agrupadosPorAnio).sort((a, b) => b - a).map(anio => {
        ...
        const isOpenAnio = (anio === anioActual || hayFiltroActivo) ? 'open' : '';
        ...
        const mesesHtml = mesesOrdenados.map(mes => {
            ...
            // AHORA NACEN SIEMPRE ABIERTOS
            const isOpenMes = 'open';
        ...
        <div class="year-group ${isOpenAnio}">
            <div class="year-summary">
                ...
                <span class="year-count">${movimientosDelAnio.length} movimientos</span>
            </div>
...
document.addEventListener('keydown', e => {
    ...
    // ── Dentro de modal entrada / salida ─────────────────
    if (modalOpen && (modalId === 'modal-entrada' || modalId === 'modal-salida')) {
        if (tag === 'BUTTON') return;
        const tipo = modalId === 'modal-entrada' ? 'entrada' : 'salida';
        if (e.key === '+' || e.key === '=') { e.preventDefault(); agregarLinea(tipo); return; }
        if (e.key === '-') {
            e.preventDefault();
            const lineas = _lineasState[tipo].lineas;
        ...
        if (e.key === 'Enter' && tag !== 'TEXTAREA') { e.preventDefault(); guardarMovimiento(tipo); return; }
    }
...
// ═══════════════════════════════════════════════════════
//  BOTÓN SUBIR ARRIBA (SCROLL)
// ═══════════════════════════════════════════════════════
window.addEventListener('scroll', () => {
    const btn = document.getElementById('btn-scroll-top');
    if (!btn) return;

    // Si el scroll bajó más de 1 página entera de altura (window.innerHeight)
...
const GistSync = (() => {
    ...
    async function bajar() {
        ...
        try {
            ...
            const procesarBajada = () => {
                ...
                // Pre-conteo sin mutar para detectar cambios ANTES de empujar el historial
                const matIds = new Set(state.materiales.map(m => m.id));
                const movIds = new Set(state.movimientos.map(m => m.id));
                const catSet = new Set(state.categorias.map(c => c.toLowerCase()));
                const herrIds = new Set(state.herramientas.map(h => h.id)); // ── NUEVO

                let _cMats = 0, _cMovs = 0, _cCats = 0, _cHerr = 0; // ── NUEVO
                
                (remoto.materiales || []).forEach(m => {
                    const limpio = _sanitizarMaterial(m); if (!limpio) return;
                    if (!matIds.has(limpio.id)) { _cMats++; }
                    else {
                        const ex = state.materiales.find(x => x.id === limpio.id);
                        if (ex && ((ex.umbralBajo === null && limpio.umbralBajo !== null) || (ex.umbralAlto === null && limpio.umbralAlto !== null))) _cMats++;
                    }
                });
                (remoto.movimientos || []).forEach(m => { const l = _sanitizarMovimiento(m); if (l && !movIds.has(l.id)) _cMovs++; });
                (remoto.categorias || []).forEach(c => { const l = _sanitizarCategoria(c); if (l && !catSet.has(l.toLowerCase())) _cCats++; });
                (remoto.herramientas || []).forEach(h => { const l = _sanitizarHerramienta(h); if (l && !herrIds.has(l.id)) _cHerr++; }); // ── NUEVO

                if (_cMats === 0 && _cMovs === 0 && _cCats === 0 && _cHerr === 0) { // ── NUEVO
                    _cfg.token = token;
                    _cfg.gistId = gistId;
                    _cfg.lastSync = new Date().toISOString();
                    _guardarCfg();
    ...
    async function verificarAlAbrir() {
        ...
        try {
            ...
            const matNuevos = (remoto.materiales || []).filter(m => !matIds.has(m.id));
            const movNuevos = (remoto.movimientos || []).filter(m => !movIds.has(m.id));
            const catNuevos = (remoto.categorias || []).filter(c => !catSet.has(c.toLowerCase()));
            const herrNuevos = (remoto.herramientas || []).filter(h => !herrIds.has(h.id)); // ── NUEVO

            // ── NUEVO: Validación incluyendo herrNuevos ──
            if (!matNuevos.length && !movNuevos.length && !catNuevos.length && !herrNuevos.length) return; 
            ...
            if (detalle) {
                const chips = [];
                if (matNuevos.length) chips.push(`<div class="gist-novedades-chip"><span class="gist-novedades-chip-label">Materiales</span><span class="gist-novedades-chip-count">+${matNuevos.length}</span></div>`);
                if (movNuevos.length) chips.push(`<div class="gist-novedades-chip"><span class="gist-novedades-chip-label">Movimientos</span><span class="gist-novedades-chip-count">+${movNuevos.length}</span></div>`);
                if (catNuevos.length) chips.push(`<div class="gist-novedades-chip"><span class="gist-novedades-chip-label">Categorías</span><span class="gist-novedades-chip-count">+${catNuevos.length}</span></div>`);
                // ── NUEVO: Chip visual para herramientas ──
                if (herrNuevos.length) chips.push(`<div class="gist-novedades-chip"><span class="gist-novedades-chip-label">Herramientas</span><span class="gist-novedades-chip-count">+${herrNuevos.length}</span></div>`);
...
<div class="page">
  ...
  <div class="rpt-footer">Materiales · Reporte generado el ${fechaReporte} · ${esc(labelPeriodo)}</div>
    ...
    if (!ventana) { toast('El navegador bloqueó la nueva pestaña. Permitila en la barra de direcciones.', 'error'); return; }
...
(function init() {
    ...
    // Acciones rápidas
    _on('btn-abrir-entrada', 'click', () => abrirModalMovimiento('entrada'));
    _on('btn-abrir-salida', 'click', () => abrirModalMovimiento('salida'));
    ...
    // Modal ajustes
    _on('ajustes-btn-nuevo-material', 'click', () => { MM.cerrar('modal-ajustes'); setTimeout(() => abrirModalMaterial(null, 'ajustes'), 150); });
    _on('ajustes-btn-reporte', 'click', () => UI.abrirReporte());
    _on('ajustes-btn-categorias', 'click', () => UI.abrirCategorias());
    _on('ajustes-btn-herramientas', 'click', () => UI.abrirHerramientas());
    ...
    // Modal herramientas
    _on('herr-agregar-btn', 'click', () => agregarHerramienta());
    ...
    // dark mode
    try {
        if (localStorage.getItem('mat_dark') === '1') {
    ...
    try {
        const lastTime = parseInt(localStorage.getItem('mat_tab_time') || '0');
        const now = Date.now();

        // Si pasaron menos de 1 hora (3.600.000 ms), respetamos la pestaña guardada
        if (now - lastTime <= 3600000) {
            const t = localStorage.getItem('mat_tab');
            if (t === 'movimientos') {
            ...
            localStorage.setItem('mat_tab', 'dashboard');
        }

        // Renovamos el temporizador desde "ahora"
        localStorage.setItem('mat_tab_time', now.toString());
    } catch (_) { }
